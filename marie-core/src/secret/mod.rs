...
#[derive(Clone)]
pub struct NodeSecretManager {
    global: SecretManager,
    epoch: KeyEpoch,
    peer_key: [u8; 32],
}

impl SecretCodec for NodeSecretManager {
...
impl SecretManager {
    ...
    pub fn for_node(&self, node_id: impl Into<NodeId>) -> SecretResult<NodeSecretManager> {
        self.for_node_epoch(node_id.into(), self.current_epoch())
    }
    ...
    pub fn for_node_epoch(&self, node_id: NodeId, epoch: KeyEpoch) -> SecretResult<NodeSecretManager> {
        let peer_key = self.derive_node_key_for_epoch(epoch, &node_id)?;
        Ok(NodeSecretManager { global: self.clone(), epoch, peer_key })
    }
    ...
    pub fn derive_node_key(&self, node_id: &NodeId) -> SecretResult<SecretKey> {
        self.derive_node_key_for_epoch(self.current_epoch(), node_id)
    }

    /// Comme [`Self::derive_node_key`], sous une epoch spécifique — voir
    /// [`Self::for_peer_epoch`].
    pub fn derive_node_key_for_epoch(&self, epoch: KeyEpoch, node_id: &NodeId) -> SecretResult<SecretKey> {
        self.derive_key(epoch, node_id.as_ref())
    }
    ...
    pub fn prove_membership(&self, node_id: &NodeId, nonce: &[u8]) -> SecretResult<(KeyEpoch, [u8; 32])> {
    ...
    /// invalide.
    pub fn verify_membership(&self, node_id: &NodeId, nonce: &[u8], epoch: KeyEpoch, proof: &[u8]) -> SecretResult<bool> {
