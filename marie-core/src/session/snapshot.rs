...
use crate::{session::{SessionId, channel::ChannelName, controller::SessionError, frames::FrameId, store::SessionStore}, store::PgStore};
...
impl SnapshotContainer {
    ...
    /// `FrameTree::arena` : point d'entrée unique vers Postgres, un cliché
    /// n'existe dans le cache qu'enveloppé ici, jamais comme `Snapshot` nu.
    pub async fn new(store: PgStore, session_id: SessionId, frame_id: FrameId, superstep: u32) -> Result<Self, sqlx::Error> {
    ...
    /// [`Self::from_snapshot`].
    fn from_loaded(store: PgStore, snapshot: Snapshot) -> Self {
    ...
    /// cache.
    fn from_snapshot(store: PgStore, snapshot: Snapshot) -> Self {
...
/// [`SessionStore::upsert_snapshot`]) plutôt que par `frame_id` seul,
...
impl Snapshots {
    pub fn new(store: PgStore, session_id: SessionId)-> Self {
        Self {
            store,
            session_id,
    ...
    /// [`SessionStore::latest_snapshot`]). Contrairement à
