...
-- en complément de `marie_session_frames` (voir 0005) : un frame a un seul
-- état courant, mais accumule un cliché par `superstep` au fil de son
-- exécution (voir `Snapshot::superstep`), d'où sa propre table plutôt qu'une
-- colonne de plus sur `marie_session_frames`.
--
...
-- même frame. `superstep` est stocké en BIGINT (Postgres n'a pas d'entier
-- non signé) alors que le domaine le porte en `u32` (voir
-- `Snapshot::superstep`) — seul `data`, qui porte le `Snapshot` sérialisé
-- tel quel, garde le type d'origine.
